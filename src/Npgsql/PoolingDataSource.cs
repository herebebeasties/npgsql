...
class PoolingDataSource : NpgsqlDataSource
{
        ...
        NpgsqlConnection conn, NpgsqlTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        return numConnectors.HasValue 
            ? OpenNewConnectorCore(conn, timeout, async, numConnectors.Value, cancellationToken) 
        ...
        NpgsqlConnection conn, TimeSpan timeout, bool async, CancellationToken cancellationToken)
    {
        var numConnectors = GetNewConnectorNumber();
        return numConnectors.HasValue 
            ? OpenNewConnectorCore(conn, new NpgsqlTimeout(timeout), async, numConnectors.Value, cancellationToken) 
            : new ValueTask<NpgsqlConnector?>((NpgsqlConnector?)null);
    }
    ...
    async ValueTask<NpgsqlConnector?> OpenNewConnectorCore(
